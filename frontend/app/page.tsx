...
export default function Home() {
    ...
    useEffect(() => {
        // URLのフラグメント（#token=...）からGoogle認証後のJWTを受け取る
        const hashParams = new URLSearchParams(window.location.hash.substring(1));
        const tokenFromHash = hashParams.get("token");
        const userIdFromHash = hashParams.get("user_id");

        if (tokenFromHash && userIdFromHash) {
            localStorage.setItem("sentimental_access_token", tokenFromHash);
            localStorage.setItem("sentimental_user_id", userIdFromHash);
            // URLからフラグメントを消去（見た目を綺麗にするため）
            ...
            <div className="absolute inset-0 pointer-events-none z-0">
                {floatingEmojis.map((obj) => (
                    (!obj.latitude || !obj.longitude) ? (
                        <div 
                            key={obj.id} 
                            className="animate-float text-5xl md:text-6xl bottom-10 absolute"
                            style={{ left: `${obj.left}%` }}
                        >
                            {obj.emoji}
                        </div>
                ...
                {isMenuOpen && (
                    <div className="absolute right-0 mt-2 w-64 bg-white/95 backdrop-blur-md rounded-2xl shadow-xl border border-gray-100 p-3 flex flex-col gap-2">
                        {!isLoggedIn ? (
                            ...
                            <>
                                <p className="text-xs font-bold text-gray-400 mb-1 px-2 tracking-wider">MENU</p>
                                <button 
                                    onClick={() => window.location.href = '/profile'}
                                    className="w-full px-4 py-3 bg-blue-50 text-blue-600 hover:bg-blue-100 font-semibold rounded-xl text-sm transition-all flex items-center gap-3"
                                >
                                ...
                                <button 
                                    onClick={() => {
                                        ...
                                        localStorage.removeItem("sentimental_user_id");
                                        window.location.reload();
                                    }}
                                    className="w-full px-4 py-3 bg-white border border-gray-200 hover:bg-red-50 hover:text-red-600 text-gray-700 font-semibold rounded-xl text-sm transition-all flex items-center gap-3"
                                >
                                    <span className="w-5 h-5 flex items-center justify-center shrink-0">🚪</span>
                                    Logout
                                </button>
                        ...
                        <form 
                            onSubmit={async (e) => {
                                ...
                                try {
                                    ...
                                    // 【最重要：ゲストIDの上書きとJWT保存】
                                    // ログイン成功時にバックエンドから発行された正規のユーザーIDとJWTをローカルストレージに保存
                                    localStorage.setItem("sentimental_user_id", data.user_id);
                                    localStorage.setItem("sentimental_access_token", data.access_token);
...
>>>>>>> 4bd6c21fa336a9254ffddb9384948f939f65dcc4
}